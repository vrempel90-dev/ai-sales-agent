...
class ThreadsBrowserLayer:
    ...
    def _short_browser_error(self, exc: Exception) -> str:
        ...
        lowered = raw.lower()
        if "using playwright sync api inside the asyncio loop" in lowered:
            return "playwright_sync_api_in_async_loop"
    ...
    async def check_browser_ready(self) -> BrowserStatus:
        ...
        if ready and self._page is None:
            launched, reason = await self.load_session()
            ready = launched
            if not launched and not self.last_browser_error:
                self.last_browser_error = reason
        if ready:
            can_open, reason = await self.open_threads_home()
            login_state = "home_opened" if can_open else reason
            if not can_open:
                ready = False
        ...
        return BrowserStatus(
            ...
            live_dm_ready=False,
            can_open_threads_home=can_open,
            login_state=login_state,
            last_browser_error=self.last_browser_error,
            stopped_reason=self.stopped_reason,
        )

    async def load_session(self) -> tuple[bool, str]:
        if not self.enabled:
        ...
        try:
            ...
            if self.user_data_dir:
                ...
                if self.cookies_json.strip():
                    await self._context.add_cookies(json.loads(self.cookies_json))
    ...
    async def open_threads_home(self) -> tuple[bool, str]:
        if self._page is None:
            ok, reason = await self.load_session()
            if not ok:
                return False, reason
        try:
            ...
            if blocking:
                if blocking == "login_issue":
                    return False, "login_required"
                return False, blocking
            return True, "ok"
        except Exception as exc:
            if "timeout" in exc.__class__.__name__.lower() or "timeout" in str(exc).lower():
                self.last_browser_error = "threads_home_timeout"
                return False, "threads_home_timeout"
            self.last_browser_error = self._short_browser_error(exc).replace("browser launch failed", "page_goto_failed")
            return False, "page_goto_failed"

    async def detect_blocking_state(self):
        if self._page is None:
            return None
        try:
            text = ((await self._page.content() or "") + " " + (await self._page.title() or "")).lower()
        except Exception as exc:
            if "timeout" in exc.__class__.__name__.lower() or "timeout" in str(exc).lower():
                self.last_browser_error = "timeout"
                return "timeout"
            self.last_browser_error = self._short_browser_error(exc)
            return "interface_changed"
        for state, patterns in BLOCKING_PATTERNS.items():
            if any(p in text for p in patterns):
                normalized = "session_expired" if state == "login_issue" and "session expired" in text else state
                self.stopped_reason = normalized
                return normalized
        return None

    async def search_threads(self, keyword: str):
        if self._page is None:
            ok, reason = await self.open_threads_home()
            if not ok:
                self.stopped_reason = reason
                return []
        try:
            await self._page.goto(f"https://www.threads.net/search?q={quote_plus(keyword)}", wait_until="domcontentloaded", timeout=30000)
            if await self.detect_blocking_state():
                return []
            return await self.collect_candidate_threads(10, keyword)
        except Exception as exc:
            if "timeout" in exc.__class__.__name__.lower() or "timeout" in str(exc).lower():
                self.last_browser_error = "timeout"
                return []
    ...
    async def collect_candidate_threads(self, limit: int = 10, keyword: str = ""):
        if self._page is None:
            return []
        try:
            links = await self._page.locator("a[href]").evaluate_all("els => els.map(a => ({href: a.href, text: a.innerText || ''}))", timeout=10000)
        except Exception as exc:
            if "timeout" in exc.__class__.__name__.lower() or "timeout" in str(exc).lower():
                self.last_browser_error = "selector_missing"
    ...
    async def open_thread(self, url: str):
        if self._page is None:
    ...
    async def read_thread_context(self):
        if self._page is None:
            return ""
        try:
            return (await self._page.locator("body").inner_text(timeout=5000))[:4000]
        except Exception as exc:
            if "timeout" in exc.__class__.__name__.lower() or "timeout" in str(exc).lower():
                self.last_browser_error = "selector_missing"
                self.stopped_reason = "interface_changed"
    ...
    async def publish_comment(self, thread: dict | ThreadCandidate, comment: str):
        if self._page is None:
            return False, "browser_not_started"
        blocking = await self.detect_blocking_state()
        if blocking:
        ...
        try:
            ...
            await box.fill(comment, timeout=5000)

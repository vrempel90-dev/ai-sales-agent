...
SALON_MARKERS = ("салон", "барбершоп", "парикмах", "маникюр", "косметолог")
...
def build_lead_reply(message: str, contact_link: str = "", phone: str = "") -> LeadReply:
    ...
    if stage == "business_described":
        ...
        if _contains(business.lower(), SALON_MARKERS):
            return LeadReply(
                "Понял, для салона это как раз сильный кейс.\n\n"
                "Обычно там теряются заявки в трёх местах: админ долго отвечает, клиент "
                "спрашивает цену и пропадает, или запись не доводится до конца. В итоге "
                "оплаченный рекламой диалог уходит конкуренту.\n\n"
                "AI-бот закрывает первый этап: отвечает сразу, уточняет услугу, собирает "
                "контакт, предлагает запись и передаёт вам тёплую заявку.\n\n"
                "Сейчас больше теряете на первом ответе или на записи клиентов?",
                stage,
            )
        return LeadReply(
            f"Понял: {summary}\n\n"

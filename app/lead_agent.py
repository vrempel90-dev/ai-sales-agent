...
def build_lead_reply(message: str, contact_link: str = "", phone: str = "") -> LeadReply:
    ...
    if stage == "irrelevant_site_request":
        ...
        text = (
            "Отлично. Сначала быстро разберём задачу, после этого я скажу точную стоимость "
            "и сроки и подскажу следующий шаг."
        )
        if destination:
        ...
        text = (
            ...
            "нужна ли CRM, запись или несколько каналов.\n\n"
            "Сначала быстро разберу процесс, потом скажу точную стоимость и сроки.\n\n"
        ...
        handoff = (
            f"Похоже, задача уже предметная. Напишите в WhatsApp — там быстро разберём "
            f"процесс и я скажу точную стоимость и сроки: {destination}"
            if destination
            else (
                "Похоже, задача уже предметная. Оставьте номер или удобный контакт — "
                "быстро разберём процесс, и я скажу точную стоимость и сроки."
            )
        )
        text = f"Понял. {value}\n\n{handoff}"
